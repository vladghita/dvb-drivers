...
{
	if (!dvb_attach(si2157_attach, adap->fe_adap->fe, &si2157_cfg,
		&adap->dev->i2c_adap))
		return -EIO;
		
    info("attached tuner si2157 (added by Vlad)!");
...
static int tbs5881_frontend_attach(struct dvb_usb_adapter *d)
{
	...
	if (tbs5881_properties.adapter->fe->tuner_attach == &tbs5881_tuner_attach) {
		d->fe_adap->fe = dvb_attach(si2168_attach, &si2168_cfg,
				&d->dev->i2c_adap);
		...
		if (d->fe_adap->fe != NULL) {
			...
			info("attached si2168 (added by Vlad)!");
